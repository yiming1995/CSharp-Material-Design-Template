...
namespace AIOBOT
{
    ...
    public class HttpCommon
    {
        ...
        public delegate void setRequestHeaders(HttpCommon httpCommon);
        ...
        public HttpCommon(string siteName = "")
        {
        ...
        public void setURL(string strURL)
        {
            ...
            if (strURL.StartsWith("https", StringComparison.OrdinalIgnoreCase))
            {
                ...
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12; // 20151124_RSG SSl->Tls
            }
            else
            {
            ...
            if (CGlobalVar.g_bUseProxy)
            {
                setProxy(CGlobalVar.g_strProxyIP, CGlobalVar.g_nProxyPort, CGlobalVar.g_strProxyID, CGlobalVar.g_strProxyPass);
            }
            else
            {
                removeProxy();
        ...
        public void setProxy(string strProxyIP, string nProxyPort, string strProxyID, string strProxyPass)
        {
            var proxy = new WebProxy
            {
                Address = new Uri($"http://{strProxyIP}:{nProxyPort}"),
                BypassProxyOnLocal = false,
                UseDefaultCredentials = false,
                Credentials = new NetworkCredential(
                                       userName: strProxyID,
                                       password: strProxyPass)
            };           
